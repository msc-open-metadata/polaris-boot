...
INSTANCE_NAME="polaris-backend-$RANDOM_SUFFIX"

CREATE_DB_RESPONSE=$(az postgres flexible-server create -l $CURRENT_REGION -g $CURRENT_RESOURCE_GROUP -n $INSTANCE_NAME -u  $QUARKUS_DATASOURCE_USERNAME -p $QUARKUS_DATASOURCE_PASSWORD -y)
...
task build:opendic:polaris-postgres-admin

task docker:polaris-jdbc
